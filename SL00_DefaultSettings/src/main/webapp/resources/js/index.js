...
            var formData = new FormData(this);
            ...
            // window.contextPath 재정의 (탭 전환 시 초기화 방지)
            if (!window.contextPath) {
                window.contextPath = '';
            }

            // 폼의 action 속성에서 contextPath 추출
            var formAction = this.action;
            var contextPath = '';
            if (formAction && formAction.includes('/checkin/')) {
                contextPath = formAction.substring(0, formAction.indexOf('/checkin/'));
            }

            console.log('=== 체크인 AJAX 요청 시작 ===');
            console.log('window.contextPath:', window.contextPath);
            console.log('formAction:', formAction);
            console.log('추출된 contextPath:', contextPath);
            console.log('요청 URL:', (contextPath || window.contextPath || '') + '/checkin/lookup.htm');

            fetch((contextPath || window.contextPath || '') + '/checkin/lookup.htm', {
            ...
            .then(function(data) {
                ...
                if (data.success) {
                    ...
                    window.location.href = (contextPath || window.contextPath || '') + '/' + data.redirectUrl;
